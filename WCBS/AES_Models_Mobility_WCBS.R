...
WCBS_all_data <- wcbs_responses %>%
  ...
  left_join(WCBS_aes, by = c("buttsurv.GRIDREF_1km" = "CELLCODE","YEAR" = "Year")) %>%
  ungroup() %>%
  filter(AES1KM < 50000) %>%
  mutate(YR = as.factor(YEAR),
         AES1KM = (AES1KM-3000)/5000,
         AES3KM = (AES3KM-3000)/5000,
         TRANSECT_LENGTH_NEW = TRANSECT_LENGTH/1000,
...
Abun_WCBS_mod <- brm(Low_mobility_abund ~ AES1KM*AES3KM + 
                      ...
                      cores = 4, file = paste0(modpath,"WCBS_LM_Abundance_brm"))
...
plot(conditional_effects(Abun_WCBS_mod, effects = "AES1KM:AES3KM",
  ...
  labs(x = "AES1KM ('000s)")
ggsave("WCBS Low mobility Abundance AES1km AES3km interaction.png",
...
plot(conditional_effects(Abun_WCBS_mod, effects = "AES3KM:AES1KM",
  ...
  scale_colour_manual(values = c("#E69F00","#CC79A7","#0072B2"),
                      ...
                      labels = c(5000,2500,500)) +
  labs(x = "AES3KM ('000s)")
ggsave("WCBS Low mobility Abundance AES3km AES1km interaction.png",
       path = modpath, width = 15, height = 12, units = "cm", dpi = 600)
...
# negative binomial - with site RE
Abun_WCBS_mod <- brm(Med_mobility_abund ~ AES1KM*AES3KM + 
                        N_VISITS_MAYTOAUGUST + TRANSECT_LENGTH_NEW + YR +
                        Climate_PC1 + Landscape_PC1 + Habitat_PC1 +
                        (1|SITENO),
                      data = WCBS_all_data, family = "negbinomial", prior = mod_pr,
                      cores = 4, file = paste0(modpath,"WCBS_MM_Abundance_brm"))
summary(Abun_WCBS_mod)
plot(Abun_WCBS_mod)
pp_check(Abun_WCBS_mod)
pp_check(Abun_WCBS_mod) +
...
plot(conditional_effects(Abun_WCBS_mod, effects = "AES1KM:AES3KM",
  ...
  scale_colour_manual(values = c("#E69F00","#CC79A7","#0072B2"),
                      ...
                      labels = c(5000,2500,500)) +
  labs(x = "AES1KM ('000s)")
ggsave("WCBS Medium mobility Abundance AES1km AES3km interaction.png",
       path = modpath, width = 15, height = 12, units = "cm", dpi = 600)
...
plot(conditional_effects(Abun_WCBS_mod, effects = "AES3KM:AES1KM",
  ...
  scale_colour_manual(values = c("#E69F00","#CC79A7","#0072B2"),
                      ...
                      labels = c(5000,2500,500)) +
  labs(x = "AES3KM ('000s)")
ggsave("WCBS Medium mobility Abundance AES3km AES1km interaction.png",
       path = modpath, width = 15, height = 12, units = "cm", dpi = 600)



# negative binomial - with site RE
Abun_WCBS_mod <- brm(High_mobility_abund ~ AES1KM*AES3KM + 
                        N_VISITS_MAYTOAUGUST + TRANSECT_LENGTH_NEW + YR +
                        Climate_PC1 + Landscape_PC1 + Habitat_PC1 +
                        (1|SITENO),
                      data = WCBS_all_data, family = "negbinomial", prior = mod_pr,
                      cores = 4, file = paste0(modpath,"WCBS_HM_Abundance_brm"))
summary(Abun_WCBS_mod)
plot(Abun_WCBS_mod)
pp_check(Abun_WCBS_mod)
pp_check(Abun_WCBS_mod) +
...
plot(conditional_effects(Abun_WCBS_mod, effects = "AES1KM:AES3KM",
  ...
  scale_colour_manual(values = c("#E69F00","#CC79A7","#0072B2"),
                      ...
                      labels = c(5000,2500,500)) +
  labs(x = "AES1KM ('000s)")
ggsave("WCBS High mobility Abundance AES1km AES3km interaction.png",
       path = modpath, width = 15, height = 12, units = "cm", dpi = 600)
...
plot(conditional_effects(Abun_WCBS_mod, effects = "AES3KM:AES1KM",
  ...
  scale_colour_manual(values = c("#E69F00","#CC79A7","#0072B2"),
                      ...
                      labels = c(5000,2500,500)) +
  labs(x = "AES3KM ('000s)")
ggsave("WCBS High mobility Abundance AES3km AES1km interaction.png",
       path = modpath, width = 15, height = 12, units = "cm", dpi = 600)
